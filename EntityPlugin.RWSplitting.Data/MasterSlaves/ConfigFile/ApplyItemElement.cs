...
namespace EntityPlugin.RWSplitting.MasterSlaves
{
	...
	/// </summary>
	public class ApplyItemElement : ConfigurationElement
	{
		...
		private const string _ServerStateScanWithNonOffline_Key = "serverStateScanWithNonOffline";
			...
			: this()
		{
			if (string.IsNullOrEmpty(targetContextFullName))
				throw new ArgumentNullException(nameof(targetContextFullName));

			targetContextFullName = targetContextFullName.Trim();
		...
		/// <summary>
		/// 读取 <see cref="ApplyItemElement"/> 配置节中表示 EF 读写分离服务配置文件信息中的 查询（读取）操作（Slave 服务器节点） 数据库服务连接参数配置信息集合。
		/// </summary>
		...
		/// </summary>
		[ConfigurationProperty(_ServerStateScanWithNonOffline_Key, DefaultValue = true)]
		...
		/// <summary>
		/// 获取该 <see cref="ApplyItemElement"/> 对象所表示的目标 EF 实体数据库上下文类型的完整名称。
		/// <para>如果该对象的 <see cref="TargetContext"/> 属性未定义，则该属性将返回 null。</para>
		/// <para>如果该对象的 <see cref="TargetContext"/> 属性定义不正确，则该读取属性可能会导致异常。</para>
		/// </summary>
		...
		private bool ValidateTargetContextType(string targetContextFullName)
		{
			...
			return ValidateTargetContextType(targetContextType);
		}
