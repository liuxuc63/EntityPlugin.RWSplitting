        ...
        public virtual string ProviderName
        {
            get
            {
                if (string.IsNullOrWhiteSpace(this._providerName))
                {
                    ...
                    if (attribute == null)
                    {
                        throw new InvalidOperationException(Resource.NotDefinedDbProviderAttribute);
                    }
            ...
            lock (_locker)
            {
                if (!DbScriptParameterParserCache.TryGetValue(providerName, out parser))
                {
                    ...
                    if (type == null)
                    {
                        ...
                        {
                            throw new InvalidOperationException(string.Format(Resource.NotExistsAllowedDbProviderAttributeDbScriptParameterParser, providerName));
                        }
