using EntityPlugin.RWSplitting.Generic;
using EntityPlugin.RWSplitting.Utilities;
using System;
...
namespace EntityPlugin.RWSplitting.Common
{
	...
	public static class DatabaseFactories
	{
		...
		public static DatabaseFactory GetDefaultFactory()
		{
			...
			if (settings == null)
			{
				throw new InvalidOperationException(Resource.NotExistsDatabaseConfig);
			}
		...
		public static DatabaseFactory GetFactory(string providerName)
		{
			...
			if (!DatabaseFactoryCache.TryGetValue(providerName, out factory))
			{
				...
				if (type == null)
				{
					...
					{
						throw new ArgumentException(string.Format(Resource.NotExistsAllowedDbProviderAttributeDatabaseFactory, providerName));
					}
		...
		{
			return ConfigurationManager.ConnectionStrings.OfType<ConnectionStringSettings>().LastOrDefault();
		}
	}
}